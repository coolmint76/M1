    ...
    def calculate_t2(self):
        ...
        try:
            ...
            if self.is_thin_roi(self.roi_coords):
                ...
                self.t2_map = self.apply_septum_specific_corrections(self.t2_map, self.r2_map)
    
                # Additional diagnostics
                print(f"Septum post-processing complete")
                print(f"T2 range after correction: [{np.min(self.t2_map[self.t2_map>0]):.1f}, "
                      f"{np.max(self.t2_map):.1f}] ms")
    ...
    def update_pixel_display(self, x, y):
        ...
        if t2_value > 0:
            ...
            if self.is_thin_roi(self.roi_coords):
                ...
                reference_text = "\n\nMyocardial T2 reference:\n45-55ms at 1.5T\n50-60ms at 3T"

            self.t2_result_var.set(self.t2_result_var.get() + reference_text)
