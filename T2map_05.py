...
class T2MappingApp:
    ...
    def normalize_signal_intensities_improved(self):
        """
        Improved normalization for T2 mapping that preserves decay characteristics
        Based on peer-reviewed methods for quantitative MRI
        """
        if self.image_stack is None:
            return
        
        print("\nApplying improved signal normalization...")
        
        # Method 1: Noise-based normalization
        # Use noise regions that should be constant across all TE preps
        noise_regions = self.identify_noise_regions()
        
        if noise_regions is not None:
            # Calculate normalization based on noise statistics
            reference_noise = []
            for i, img in enumerate(self.image_stack):
                noise_values = img[noise_regions]
                # Use robust statistics
                noise_std = self.robust_std(noise_values)
                reference_noise.append(noise_std)
            
            # Normalize to first image noise level
            norm_factors = np.array(reference_noise) / reference_noise[0]
            
            print(f"Noise-based normalization factors: {norm_factors}")
            
            # Apply normalization
            for i in range(len(self.image_stack)):
                if norm_factors[i] > 0:
                    self.image_stack[i] = self.image_stack[i] / norm_factors[i]
        
        else:
            # Method 2: Muscle/Fat reference normalization
            # For cardiac/body imaging, use tissue with known long T2
            print("Attempting tissue-based normalization...")
            
            # Find a region with minimal T2 decay (long T2)
            reference_roi = self.find_stable_reference_tissue()
            
            if reference_roi is not None:
                reference_signals = []
                for img in self.image_stack:
                    ref_signal = np.mean(img[reference_roi])
                    reference_signals.append(ref_signal)
                
                # Check if reference shows minimal decay
                decay_ratio = reference_signals[-1] / reference_signals[0]
                
                if decay_ratio > 0.8:  # Less than 20% decay - good reference
                    norm_factors = np.array(reference_signals) / reference_signals[0]
                    
                    # Correct for the small amount of decay in reference
                    te_preps = self.te_preps
                    # Fit exponential to reference decay
                    ref_t2 = self.fit_reference_t2(te_preps, reference_signals)
                    
                    # Correct normalization factors
                    expected_decay = np.exp(-te_preps / ref_t2)
                    corrected_norm_factors = norm_factors / expected_decay
                    
                    print(f"Reference T2: {ref_t2:.1f} ms")
                    print(f"Corrected normalization factors: {corrected_norm_factors}")
                    
                    # Apply corrected normalization
                    for i in range(len(self.image_stack)):
                        if corrected_norm_factors[i] > 0:
                            self.image_stack[i] = self.image_stack[i] / corrected_norm_factors[i]
                else:
                    print("Reference region shows significant decay - skipping normalization")
    def normalize_signal_intensities_improved(self):
        """
        Improved normalization for T2 mapping that preserves decay characteristics
        Based on peer-reviewed methods for quantitative MRI
    ...
    def diagnose_t2_data(self):
        ...
        ax3.set_title("ROI Signal Decay (Normalized)")
        ax3.legend()
    ...
    def fit_t2_with_corrections(self, te_preps, signal, apply_noise_correction=True,
        ...
        # Remove zero values for fitting
        valid_idx = signal_corrected > 0
        if np.sum(valid_idx) < 2:
            return 0.0, 0.0, {}
        
        te_valid = te_preps[valid_idx]
        signal_valid = signal_corrected[valid_idx]
        ...
        if self.fitting_method_var.get() == "3-parameter":
            ...
            # Initial guess
            s0_guess = np.max(signal_valid) - np.min(signal_valid)
            c_guess = np.min(signal_valid)
            
            # Estimate T2 from log-linear regression
            if c_guess > 0:
                signal_minus_offset = signal_valid - c_guess
                valid_for_log = signal_minus_offset > 0
                if np.sum(valid_for_log) >= 2:
                    log_signal = np.log(signal_minus_offset[valid_for_log])
                    te_for_log = te_valid[valid_for_log]
                    slope, _ = np.polyfit(te_for_log, log_signal, 1)
            ...
            try:
                ...
                popt, pcov = opt.curve_fit(
                    t2_model, te_valid, signal_valid,
            ...
            try:
                ...
                print(f"2-parameter fit failed: {e}")
                return 0.0, 0.0, {}
    ...
    def estimate_noise_level(self):
        ...
        # Find background corners
        corner_size = min(30, img.shape[0]//10, img.shape[1]//10)
        corners = [
        ...
        # Estimate noise from low-signal corners
        noise_estimates = []
        signal_threshold = 0.1 * np.max(img)
        
        for corner in corners:
            if np.mean(corner) < signal_threshold:
                # For Rician noise in magnitude images
                noise_estimates.append(np.std(corner))
        
        if noise_estimates:
            # Correction factor for Rician distribution
    ...
    def calculate_t2(self):
        ...
        try:
            ...
            valid_mask = (self.t2_map > 0) & (self.t2_map < 200) & (self.r2_map > 0.8)
