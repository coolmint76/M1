...
class T2MappingApp:
    ...
    def normalize_signal_intensities(self):
        """Normalize signal intensities across images to correct for scaling differences"""
        if self.image_stack is None:
            return
            
        print("\nNormalizing signal intensities...")
        
        # Find a reference region (e.g., background noise)
        reference_signals = []
        for i, img in enumerate(self.image_stack):
            # Use corner regions for normalization
            corner_size = 20
            corners = [
            ...
            # Use the 75th percentile of all corners as reference
            corner_values = np.concatenate([c.flatten() for c in corners])
            ref_value = np.percentile(corner_values, 75)
            reference_signals.append(ref_value)
        
        # Normalize to the first image
