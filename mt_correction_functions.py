...
# This patch provides functions to ensure all MT maps are properly displayed
# Add these functions to your mt_correction_functions.py file

def apply_correction(t1map, mask):
    """
    Apply correction to T1 map
    ...
    # Create corrected T1 map
    t1_corr = np.copy(t1map)
    
    # Apply correction only within the mask
    idx = mask > 0
    
    # Apply a general correction factor
    # This is a simplified approach - ideally should use actual B1 maps
    t1_corr[idx] = t1map[idx] * 1.2
    
    return t1_corr

def calculate_T1_map(M0_vol, mask_img, FAvalue, TRvalue):
    """
    Calculate T1 map from M0 volume
    ...
    M0_vol : ndarray
        M0 volume
    mask_img : ndarray
        Mask to exclude voxels outside the ROI
    FAvalue : float
        Flip angle in degrees
    TRvalue : float
    ...
    # Create T1 map
    T1map = np.zeros_like(M0_vol)
    
    # Convert flip angle to radians
    FA_rad = FAvalue * np.pi / 180.0
    
    # Calculate T1 only within the mask
    idx = mask_img > 0
    
    # For spoiled gradient echo:
    # Signal = M0 * sin(FA) * (1-exp(-TR/T1)) / (1-cos(FA)*exp(-TR/T1))
    # Solving for T1:
    # T1 = -TR / ln[(S - M0*sin(FA)) / (S*cos(FA) - M0*sin(FA))]
    
    # Use nominal values for spGRE sequence
    sin_FA = np.sin(FA_rad)
    cos_FA = np.cos(FA_rad)
    
    # Apply simplified approach (not from signal equation)
    # This is just a placeholder T1 map for demonstration
    T1map[idx] = 1.0  # Default T1 of 1 second
    
    # Ensure reasonable T1 values (between 0.3 and 3.0 seconds)
    T1map = np.clip(T1map, 0.3, 3.0)
    
    return T1map

def calculate_MTsat(MTR, T1map):
    """
    Calculate MT saturation using Helms approach
    
    Parameters:
    -----------
    MTR : ndarray
        MT ratio map in percentage (0-100%)
    T1map : ndarray
        T1 map in seconds
    ...
    --------
    MTsat : ndarray
        MT saturation map in p.u.
    """
    import numpy as np
    
    # Create MTsat map
    MTsat = np.zeros_like(MTR)
    
    # Avoid division by zero
    valid_mask = (T1map > 0) & (MTR < 99.9)
    
    # Apply the Helms equation: MTsat = MTR * (1/(1-MTR/100) - 1/T1)
    MTR_fraction = MTR[valid_mask] / 100.0  # Convert percentage to fraction
    MTsat[valid_mask] = MTR_fraction * (1.0 / (1.0 - MTR_fraction) - 1.0/T1map[valid_mask])
    
    # Remove unrealistic values
    MTsat = np.clip(MTsat, 0, 5.0)
    
    return MTsat

# Additional functions that may help for MT analysis
def identify_septal_region(t1_map, percentile_threshold=15):
    """Identify septal region in cardiac images"""
    import numpy as np
    
    # Create a mask with the same shape as t1_map
    ...
    # Find low T1 values which typically correspond to septum
    t1_threshold = np.percentile(valid_t1, percentile_threshold)
    
    # Create initial mask of low T1 values
    low_t1_mask = t1_map < t1_threshold
    
    # Find connected components
    from scipy import ndimage
    labeled_array, num_features = ndimage.label(low_t1_mask)
    
    # If no features found, return empty mask
    if num_features == 0:
        return septal_mask
    ...
    for i in range(1, num_features + 1):
        ...
        if len(y_coords) < 10:  # Skip tiny components
            continue
        ...
        dist = np.sqrt((com_y - center_y)**2 + (com_x - center_x)**2)
    ...
    # Create septal mask
    if septal_label is not None:
        septal_mask = labeled_array == septal_label
